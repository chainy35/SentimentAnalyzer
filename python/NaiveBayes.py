...
class NaiveBayes:
  ...
  def __init__(self):
    ...
    self.posWord = {}   #  word for positve reviews and its occurence
    self.negWord = {}   #  word for negative reviews and its occurence
    self.allWord = {}     # word for all reviews
    self.numPosWords = 0.0    # total number of words in positve reviews
    self.numNegWords = 0.0    # total number of words in negatvie reviews
  ...
  def classify(self, words):
    ...
    pPos = math.log(self.numPosReviews / numTotalReviews)  
    pNeg = math.log(self.numNegReviews / numTotalReviews)  


    for word in words:
      pPos += math.log((self.posWord.get(word,0) + 1)/(self.numPosWords + len(self.allWord) + 1)) 
      pNeg += math.log((self.negWord.get(word,0) + 1)/(self.numNegWords + len(self.allWord) + 1)) 
  ...
  def addExample(self, klass, words):
    ...
    if klass == 'pos':
      ...
      for word in words:             
        self.posWord[word] = self.posWord.get(word,0) + 1
        self.numPosWords += 1
        self.allWord[word] = self.allWord.get(word,0) + 1
    else:
      self.numNegReviews += 1
      for word in words:             
        self.negWord[word] = self.negWord.get(word,0) + 1
        self.numNegWords += 1
        self.allWord[word] = self.allWord.get(word,0) + 1
